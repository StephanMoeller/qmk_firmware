...
// Ported from qmk_firmware_old/keyboards/rollercole/keymap.c. Unlike the
// v0_2 port, v0_6's top/home/bottom rows are all full width (10/10/10),
// matching what the old file assumed, so nothing had to be dropped there.
// The only physical gap is the thumb cluster: old assumed 3 keys per side,
// v0_6 has 1. The old design's outer thumb keys were MO(F_KEYS) and the
// middle ones were duplicate wiring for THUMB_LEFT/THUMB_RIGHT, so:
//   - outer thumb keys now just send A / B as plain key presses
//   - inner thumb keys carry THUMB_LEFT / THUMB_RIGHT (unchanged logic)
// F_KEYS lost its dedicated activation key, so it's reachable instead by
// holding both outer thumb keys (A + B) together as a momentary layer.
...
enum layers {
    ...
    NUMBERS,
    F_KEYS,
    BS_LAYER,  // Both (inner) thumbs held: backspace variants
    BOOT_LAYER,
    APP_LAUNCH,
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [BASE] = LAYOUT(
        ...
        KC_Z,                 KC_X,        KC_C,       KC_D,       KC_V,           KC_TRNS,    KC_H,      DK_COMM,    LT(APP_LAUNCH, DK_DOT), DK_MINS,
                                                         THUMB_A,    THUMB_LEFT,                THUMB_RIGHT,  THUMB_B
    ),
    [ARROW] = LAYOUT(
        ...
        DK_BSLS,     DK_HASH,   DK_LBRC,    DK_RBRC,    DK_AMPR,        DK_PIPE,    KC_TAB,    DK_DQUO,    DK_COLN,    DK_PLUS,
    ...
    [NUMBERS] = LAYOUT(
        ...
        KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,     KC_DEL,         TILDE,      KC_1,      KC_2,       KC_3,       KC_TRNS,
                                                         KC_TRNS,    KC_TRNS,                   KC_TRNS,      KC_TRNS
    ),
    [F_KEYS] = LAYOUT(
        KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,        KC_TRNS,    KC_F7,     KC_F8,      KC_F9,      KC_F10,
        KC_TRNS,    KC_LALT,    KC_LCTL,    KC_LSFT,    KC_TRNS,        KC_TRNS,    _S(KC_F4), _C(KC_F5),  _A(KC_F6),  KC_F11,
        KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,        KC_TRNS,    _W(KC_F1), KC_F2,      KC_F3,      KC_F12,
                                                         KC_TRNS,    KC_TRNS,                   KC_TRNS,      KC_TRNS
    ),
    [BS_LAYER] = LAYOUT(
        KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,        KC_TRNS,    KC_NO,     KC_NO,      KC_NO,      KC_TRNS,
        KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,        KC_TRNS,    _S(KC_BSPC), _C(KC_BSPC), _A(KC_BSPC), KC_TRNS,
        KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,    KC_TRNS,        KC_TRNS,    KC_DEL,    KC_DEL,     KC_DEL,     KC_TRNS,
                                                         KC_TRNS,    KC_TRNS,                   KC_TRNS,      KC_TRNS
    ),
    [BOOT_LAYER] = LAYOUT(
    ...
    [APP_LAUNCH] = LAYOUT(
        ...
        KC_TRNS,    KC_TRNS,    LGUI(KC_3), LGUI(KC_8), KC_TRNS,        KC_TRNS,    KC_TRNS,   KC_TRNS,    KC_TRNS,    KC_TRNS,
                                                         KC_TRNS,    KC_TRNS,                   KC_TRNS,      KC_TRNS
    ),
};
...
const uint16_t PROGMEM combo_fkeys[] = {THUMB_A, THUMB_B, COMBO_END};

combo_t key_combos[] = {
    [C_J]     = COMBO(combo_j, KC_J),
    [C_AE]    = COMBO(combo_lo, DK_AE),
    [C_ARNG]  = COMBO(combo_ou, DK_ARNG),
    [C_OSTR]  = COMBO(combo_lu, DK_OSTR),
    [C_FKEYS] = COMBO_ACTION(combo_fkeys),
};

void process_combo_event(uint16_t combo_index, bool pressed) {
    if (combo_index == C_FKEYS) {
        set_layer_state(F_KEYS, pressed);
    }
}
